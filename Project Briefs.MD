...
|무엇을 만들고자 하는가| 이미지 인식, 텍스트 인식을 활용하여 알약을 인식하고, OCR 및 이미지 분석을 통해 현재 복용 중인 약의 정보를 제공하는 동시에 중복 복용 및 같이 복용하면 안 되는 약을 자동으로 판별합니다. 또한, 복약 시간을 알람으로 설정하여 사용자가 약을 잊지 않고 정확한 시간에 복용할 수 있도록 돕고, 처방받은 약이 소진되기 전에 병원 방문을 미리 안내합니다.  |
|고객| 여러 병원을 다니고 중복 복용의 위험이 있는 환자 |
|Pain Point| - 처방받은 약과 함께 먹으면 위험한 약을 판단하기 어려움
- 약 포장지를를 잃어버릴 경우 무슨 약인지 판단하기가 쉽지 않음
...
|사용할 소프트웨어의 구체적 명세(명칭과 URL)| - OCR 모델: Tesseract OCR https://github.com/tesseract-ocr/tesseract 
- AI 모델: YOLO (You Only Look Once) https://github.com/AlexeyAB/darknet, OpenCV (https://opencv.org/)
