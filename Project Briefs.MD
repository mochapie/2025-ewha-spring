...
|고객|  -어린 자녀를 양육하는 맞벌이 부부
자녀 연령대: 4~8세
가치관: 창의적 교육과 정서 발달을 중요하게 생각함.
생활 패턴: 두 부부 모두 직장에서 바빠 자녀에게 많은 시간과 에너지를 투자하기 어려움.
요구사항: 자녀의 창의력과 문제 해결 능력을 키울 수 있는 다양한 활동이나 교육적 자원을 찾고 있음. 시간이 부족한 상황에서 쉽게 활용할 수 있는 콘텐츠나 프로그램을 선호.

-유치원 또는 어린이집의 교육자 
직업: 유치원 교사
가치관: 아이들의 창의력을 증진할 수 있는 학습, 가정과 연계해서 아이의 성장과 발달을 돕는 것이 중요하다고 생각함.
필요: 기존의 단순한 교구나 활동으로는 집중력이 떨어지기에 새롭고 재미있는 방식으로 아이들의 호기심을 자극하는 콘텐츠가 필요
요구사항: 가정에서 부모와 아이와 함께할 수 있는 체험형 학습 콘텐츠를 찾고 있음. 그림 그리는 아이들, 글 쓰기를 좋아하는 아이들을 위해 특별한 학습을 하고 싶음.  |
|Pain Point|  -맞벌이 부부 (양육자)
바쁜 일정으로 인해 자녀에게 충분한 교육적 케어를 해주지 못함
자녀의 창의력과 정서 발달을 지원할 수 있는 효과적이고 간편한 방법을 찾기 어려움

-유치원 또는 어린이집의 교육자
창의력과 문제해결 능력을 효과적으로 키울 수 있는 맞춤형 콘텐츠의 부재. 가정과 연계해서 참여할 수 있는 효과적인 교육 프로그램 및 자료 부족

- 기존 동화 생성 애플리케이션의 한계 극복
전체 줄거리와 이미지를 생성해 주기 때문에 창작 과정에서 아이의 창의력 발휘 기회 부족, 부모와의 정서적 교류 기회 부족한 문제 발생  |
|기술 솔루션 서술|  [Backend]
- SpringBoot, Spring Security, Spring Data JPA, Spring Web, Oauth 2.0
- AWS EC2, AWS S3, AWS RDS, Docker
...
[협업 툴]
- Git, Github, Figma, Notion, Discord, Slack

[AI, API]
- Fast API : AI 모델과의 연동을 원활하게 구현한다.
- GPT-4o : 동화의 줄거리를 추천해주고, AI 기반 퀴즈를 생성한다.

- Hugging Face Diffusers API를 활용하며, 세부적으로 ControlNet, LoRA, Stable Diffusion을 사용하여 이미지를 세부적으로 보정하고 생성한다.
- ControlNet : 아이의 밑그림을 기반으로 선을 따서 자연스럽에 이미지에 색채를 더하고 보정한다.
- LoRA : 초기 생성한 주인공 이미지를 기반으로 그림체에 일관성을 부여한다.
- Stable Diffusion : 전체적인 동화의 분위기(빛, 그림자) 미세 조정한다.

- ElevenLabs : 부모의 음성을 복제하여 동화 스크립트를 읽어준다.  |
|구현기술 키워드|  Image-to-Image Translation, NLP, TTS  |
|사용할 소프트웨어의 구체적 명세(명칭과 URL)|  - GPT-4o : https://chatgpt.com/ 
- Hugging Face Diffusers : https://huggingface.co/docs/diffusers/index
- ElevenLabs https://elevenlabs.io/docs/overview?utm_source=chatgpt.com  |
