...
|기술 솔루션 서술| - 사용자 친화적인 웹 UI를 React로 개발하여 사진 업로드, 검색, 제품 비교, 챗봇 인터페이스 등을 제공한다. <br>- 백엔드로 Spring Boot 기반의 API 서버를 구축하여 프론트엔드와 AI 모델, 데이터베이스, 공공 데이터 API 간의 연결을 담당한다. 또한 AWS 클라우드 환경에서 서버를 운영하여 확장성과 안정성을 확보한다. <br>- 사용자 맞춤 제품 추천을 위해 데이터베이스로 Firebase에 사용자의 검색 이력과 제품 선호도를 저장하고, 실시간 데이터 동기화를 활용하여, 새로운 규제 정보나 업데이트된 제품 정보를 즉시 반영할 수 있도록 한다. <br>- 공공 데이터 API에서 제공되지 않는 정보를 확보하기 위해 Scrapy를 이용해 웹 크롤링을 수행한다. <br>- 식약처, 환경부, 공정위의 웹사이트에서 제공하는 공공 데이터를 연동하여 제품 성분, 리콜 정보 등의 데이터를 정기적으로 수집 및 업데이트한다. <br>- 제품 이미지 인식을 위해 Google Cloud Vision을 활용하여 제품 패키지에서 브랜드명, 제품명, 성분표를 추출한다. <br>- 사용자가 특정 성분이나 제품의 안전성에 대해 질문하면, GPT-4o 기반의 AI 챗봇이 공공 데이터와 학습된 내용을 기반으로 답변한다. |
