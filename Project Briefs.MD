...
|기술 솔루션 서술| 1. **사용자 생활패턴과 주거공간을 고려한 맞춤형 초기 청소 스케줄 생성**<br> &nbsp;&nbsp;&nbsp;1) pulp를 사용하여 Integer Linear Programming (ILP) 기반으로 청소 시간 배정<br>&nbsp;&nbsp;&nbsp;2) 사용자의 고정 일정을 피하고, 모든 청소 활동의 반복 주기를 최대한으로 충족하는 최적해를 구함<br><br>  2. **사용자의 청소 루틴 달성률을 반영한 청소 스케줄 맞춤형 개선**&nbsp;&nbsp;&nbsp;1) ILP 방식으로 생성한 스케줄을 Reinforcement Learning (RL) 환경의 초기 상태로 설정<br>&nbsp;&nbsp;&nbsp;2) tianshou로 사용자의 실제 청소 완료 기록과 피드백을 반영하여 청소 수행률을 높이는 방향으로 스케줄 조정<br>&nbsp;&nbsp;&nbsp;3) DQN (Deep Q-Network) 알고리즘 활용<br><br>3. **날씨를 고려한 청소 루틴 추천**<br>&nbsp;&nbsp;&nbsp;1) 기상 데이터 기반 동적 청소 추천<br>&nbsp;&nbsp;&nbsp;- OpenWeatherMap 등의 API를 활용하여 실시간 날씨 데이터를 가져옴<br>&nbsp;&nbsp;&nbsp;- 미세먼지, 습도, 강수량 등 날씨 요소를 분석하여 적절한 청소 루틴 추천<br>&nbsp;&nbsp;&nbsp;- 예: 미세먼지가 높으면 창문 청소 및 환기 최소화, 습도가 높으면 욕실 곰팡이 방지 청소 우선<br>&nbsp;&nbsp;&nbsp;- ILP 및 RL 기반 최적 청소 일정 조정<br>&nbsp;&nbsp;&nbsp;- 기존 ILP 스케줄링 모델에 날씨 데이터를 반영하여 가중치 조정 |
